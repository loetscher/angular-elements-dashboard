  ...
  public initClient(): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      if (this.config.usedAsWebComponent) {
        const fontFaceElements = document.querySelectorAll(`[${agGridFontFaceMarker}='${agGridVersion}']`);
        if (fontFaceElements.length === 0) {
          ...
          fontFaceStyle.setAttribute(agGridFontFaceMarker, agGridVersion);
